import tkinter
import btnhover
from PIL import ImageTk,Image,ImageOps,ImageEnhance,ImageGrab,ImageFilter,ImageMath
import PIL
import cv2 as cv
...
class Image:
    ...
    #Opening Image
    def reset(self,widget,canvas,ogimg):
    ...
    #Save as(1. hello.jpg  2. select type jpg 3. save)10-dec-23
    ...
    def DrawOnImg(self,canvas,widget):
        global colorbox, drawimage
        drawimage=self.img.copy()
        for items in widget.winfo_children():
            items.destroy()
        ...
        try:
            ...
            colorsize = tkinter.Scale(widget, from_=1, to=60,label=' set size ',length=170, orient='horizontal',
                              bg='#383838',fg='white',command=self.pensize)
            colorsize.grid(row=2,column=0,pady=15,padx=10,columnspan=3)
        resetbtn = tkinter.Button(widget, text = "Reset",command = lambda:self.reset(widget,canvas,drawimage))
        resetbtn.grid(row=3,column=0,pady=15,padx=10,columnspan=3)
    ...
    def doTransform(self,canvas,widget):
        ...
        resizebtn=tkinter.Button(widget,text = "resize",font=font.Font(size=8, weight='bold'),command=lambda:self.doResize(wText.get(),hText.get()))
        resizebtn.grid(row=3,column=0,columnspan=2,padx=10,pady=5)
        tkinter.Label(widget,bg='#383838',text = "-------------------------------",fg='white').grid(row=4,column=0,columnspan=2,pady=10)
        rotateLabel = tkinter.Label(widget,bg='#383838',text = "Rotate",fg='white', font=font.Font(size=15, weight='bold'))
        rotateLabel.grid(row=5,column=0,padx=10,pady=20,columnspan=2)
        lr = tkinter.Button(widget, bg='#383838',text = "⤺",fg='white',font=font.Font(size=15, weight='bold'),command=lambda:self.doRotate(90,wText,hText))
        lr.grid(row=6,column=0,padx=10,pady=5)
        rr = tkinter.Button(widget, bg='#383838',text = "⤼",fg='white',font=font.Font(size=15, weight='bold'),command=lambda:self.doRotate(-90,wText,hText))
        rr.grid(row=6,column=1,padx=10,pady=5)
        tkinter.Label(widget,bg='#383838',text = "-------------------------------",fg='white').grid(row=7,column=0,columnspan=2,pady=10)
        flipLabel = tkinter.Label(widget,bg='#383838',text = "Flip",fg='white', font=font.Font(size=15, weight='bold'))
        flipLabel.grid(row=8,column=0,padx=10,pady=20,columnspan=2)
        hflip = tkinter.Button(widget, bg='#383838',text = "⬄",fg='white',font=font.Font(size=15, weight='bold'),command=lambda:self.doFlip("h"))
        hflip.grid(row=9,column=0,padx=10,pady=5)
        vflip = tkinter.Button(widget, bg='#383838',text = "⇳",fg='white',font=font.Font(size=15, weight='bold'),command=lambda:self.doFlip("v"))
        vflip.grid(row=9,column=1,padx=10,pady=5)

    def doResize(self,w,h):
        self.w=int(w)
        self.h=int(h)
    ...
    def doRotate(self,angle,wwidget,hwidget):
        temp=self.w
        self.w=self.h
        self.h=temp
        wwidget.delete(0, 'end');hwidget.delete(0, 'end')
        wwidget.insert(0, self.w)
        hwidget.insert(0,self.h)
    ...
    def doFilters(self,canvas,widget):
        ...
        f9=tkinter.Button(widget,width=15,height=2,text="Sketch",command=lambda:sketchfilter())
        f9.grid(row=9,column=0,padx=10,pady=10)
        ...
        def sketchfilter():
            tempimg = filteredimage.copy()
            numpy_image=numpy.array(tempimg)  
            tempimg2=cv.cvtColor(numpy_image, cv.COLOR_RGB2BGR)
            img_gray = cv.cvtColor(tempimg2, cv.COLOR_BGR2GRAY)
            img_invert = cv.bitwise_not(img_gray)
            img_smoothing = cv.GaussianBlur(img_invert, (21, 21),sigmaX=0, sigmaY=0)
            sketch=cv.divide(img_gray, 255 - img_smoothing, scale=256)
            self.img = PIL.Image.fromarray(sketch)
            img1 = ImageTk.PhotoImage(self.img)
            w1,h1=self.img.size
            self.ImageEdit.configure(width=w1,height=h1)
            self.ImageEdit.create_image((w1/2)+1.5,(h1/2)+1.5,image=img1)
    ...
    def __init__(self):
        ...
        #self.root.geometry('1920x1080')#'1320x920'
        ...
        self.filemenu.add_command(label="Save", command=lambda:self.doSave())
